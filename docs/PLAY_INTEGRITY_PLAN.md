# Play Integrity Plan - Chameleon

Date: 2026-07-01
Status: Planning complete, implementation not started
Package scope: `chameleon24.app` public Play app. Test-only tier packages (`chameleon24.app.free`, `.pro`, `.elite`) are for local/internal validation and should not drive public Play policy.

## Goal

Use Play Integrity as a backend risk signal for activation, access, and abuse-sensitive Chameleon actions. Do not block app startup, local decoy/private-zone UX, or existing local protections based on a missing or weak verdict in the first release.
...
- Access-tier refresh.
- Future geofence/private-zone remote policy sync.
- Suspicious high-rate backend calls from the same device.
...
- Use app access risk as a warning/step-up signal before sensitive remote policy changes.

Hard denial should require separate Gio sign-off and observed telemetry. Avoid using Play Integrity to block local privacy/decoy workflows that must remain available during network or Play Services outages.
...
```json
{
  ...
  "clientId": "chameleon-device-id",
  "packageName": "chameleon24.app"
}
```
...
- Link the Chameleon Play app to a Google Cloud project in Play Console.
