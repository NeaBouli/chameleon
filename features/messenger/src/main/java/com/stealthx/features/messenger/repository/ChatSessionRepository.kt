...
class ChatSessionRepository @Inject constructor(
    ...
    suspend fun encryptForSend(
        contact: ContactKeyEntity,
        plaintext: ByteArray,
        aad: ByteArray
    ): OutboundRatchetPayload {
    ...
    private suspend fun ChatSessionEntity.withReceiveChain(senderDhPublic: ByteArray): ChatSessionEntity {
        val sharedSecret = ChameleonCrypto.computeSharedSecret(sendDhPrivate, senderDhPublic)
        val receiveRootKey = ChameleonCrypto.hkdf(
            ikm = sharedSecret,
            salt = rootKey,
            info = INFO_ROOT_KEY,
            length = ChameleonCrypto.KEY_BYTES
    ...
    private companion object {
        ...
        val INFO_ROOT_KEY = "ChameleonRatchetRoot:v1".toByteArray(Charsets.UTF_8)
        val INFO_CHAIN_KEY = "ChameleonRatchetChain:v1".toByteArray(Charsets.UTF_8)
        val INFO_MESSAGE_KEY = "ChameleonRatchetMessage:v1".toByteArray(Charsets.UTF_8)
    }
}

private data class ChainStep(val messageKey: ByteArray, val nextChainKey: ByteArray)
