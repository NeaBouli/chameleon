...
- Payment entitlement PR #20 was squash-merged to `main` as `f03e01a`.
...
**Settings-Audit + Background Contact Listener verdrahtet**

Vollständiger Settings-/Access-Durchlauf für Chameleon:
...
- Neuer echter Settings-Schalter `Background Contact Listener`.
- Preference in `AppPreferences`; `SettingsViewModel` startet/stoppt `ContactListenerService` sofort.
- `ChameleonApplication` startet den Listener nur noch, wenn der Schalter aktiv ist.
- `BootReceiver` respektiert den Schalter und startet den ContactListener nach Reboot nicht mehr gegen das Setting.
- `ContactListenerService` stoppt Foreground/WS bei deaktiviertem Setting und prüft das auch im Reconnect-Loop.
- `ContactExchangeManager.stopListening()` ergänzt.
- Version: `versionCode=6`, `versionName=0.1.5-alpha`.

Verifikation:
- `verifyNoAppIfrWalletCode` ✅
- `app:assembleRelease app:bundleRelease` ✅
- `app:assembleInternalRelease` ✅
- Links: `https://chameleon.stealthx.tech/#lifetime` HTTP 200, `https://chameleon.stealthx.tech/wiki/user-manual.html` HTTP 200
- Installiert auf S7 + Tab S4: `chameleon24.app` v6 / `0.1.5-alpha`
- S10 nicht verbunden.
...
## 2026-06-18 16:18 PT — Codex Chameleon IFR Discount Signature Requirement

- User flagged manual wallet address fallback as insecure.
- Chameleon website IFR discount block now requires MetaMask connection and wallet signature.
...
- Settings no longer exposes app-side IFR/wallet actions; locked Pro/Elite features route to the website purchase plus activation-code flow.
- Room cache schema bumped from v2 to v3 with neutral `access_tier_cache`.
...
- Added an explicitly disabled Google Play button to the Chameleon download section:
...
External release:
- Created GitHub Release `v0.1.4-alpha-chameleon` from commit `1cb7b11`.
- Release URL:
  - `https://github.com/NeaBouli/chameleon/releases/tag/v0.1.4-alpha-chameleon`
- Uploaded assets:
  - `Chameleon-LATEST.apk` (20,150,784 bytes)
  - `Chameleon-LATEST.aab` (20,176,395 bytes)
- Verified download URLs return HTTP 200:
...
User clarified Chameleon must be IFR-free for all tiers, not only visually or only in the Free path.
...
- The task scans Android app source roots across app/core/data/domain/features/presentation/shared/access/crypto modules.
...
- Chameleon speichert das signierte Entitlement ausschliesslich in
...
## 2026-08-09 11:47 EEST — CODEX SOL — LEGACY PR #24 SELECTIVE SALVAGE

- PR #24 is stale as a branch but contains one missing security invariant: only
  `chameleon_pro_lifetime -> PRO` and `chameleon_elite_lifetime -> ELITE` are accepted.
...
`SECURITY SALVAGE TESTED — PR #24 WILL BE SUPERSEDED`

## 2026-08-09 12:02 EEST — CODEX SOL — LEGACY PR #24 SALVAGE READY

- Final diff and `git diff --check` PASS. Crypto tests remain green for both debug and release
  unit-test variants. The branch contains only product/tier binding, deterministic Sodium test
  initialization, focused tests and this append-only Bridge record.
- Ready for protected GitHub PR/CI integration. Old PR #24 will be closed as superseded after the
...
- Hosted API 26 and API 36 smoke tests passed on exact head `600c75b`, while the shared
  harness still removed its raw result before the artifact upload could collect it.
